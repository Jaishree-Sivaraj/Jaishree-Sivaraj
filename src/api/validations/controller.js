...
export const getAllValidation =async ({ user, params }, res, next) => {
  ...
  // if (categoryValidationRules.length > 0) {
    if(dpTypeValues.length > 0){
      ...
      let derivedDatapoints = await DerivedDatapoints.find({ companyId: taskDetailsObject.companyId.id, year :{$in : mergedYear},status: true }).populate('datapointId').populate('companyId');
      let kmpDatapoints = await KmpMatrixDataPoints.find({companyId: taskDetailsObject.companyId.id,isActive: true, year :{$in : mergedYear}, status: true}).populate('datapointId').populate('companyId');
      let boardMembersMatrixDataPoints = await BoardMembersMatrixDataPoints.find({companyId: taskDetailsObject.companyId.id,isActive: true, year :{$in : mergedYear}, status: true}).populate('datapointId').populate('companyId');
      let standalone_datapoints = await StandaloneDatapoints.find({ companyId: taskDetailsObject.companyId.id,  isActive: true, year :{$in : mergedYear},status: true }).populate('datapointId').populate('companyId');
