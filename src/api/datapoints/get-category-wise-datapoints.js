...
export const getCategorywiseDatapoints = async (req, res, next) => {
  try {
    ...
    let taskStartDate = getTaskStartDate(currentYear[0], fiscalYearEndMonth, fiscalYearEndDate);
        ...
        if (dpTypeValues.includes(BOARD_MATRIX) || dpTypeValues.includes(KMP_MATRIX)) {
          try {
            ...
            switch (dpType) {
              ...
              case BOARD_MATRIX:
                // let boardMember = await BoardMembers.find(dpQuery);
                // for (let currentYearIndex = 0; currentYearIndex < currentYear.length; currentYearIndex++) {
                //   const yearTimeStamp = getTaskStartDate(currentYear[currentYearIndex], fiscalYearEndMonth, fiscalYearEndDate);
                ...
                //   }
                // }
                // Getting all the users irrespective of endDateTimeStamp

                // let memberList = []
                // boardMember.map(member => {
                //   let terminatedDate = new Date(member.endDateTimeStamp * 1000);
                //   terminatedDate = format(terminatedDate, 'MM-dd-yyyy');
                //   let endDate = new Date(member.endDate).getFullYear() ? new Date(member.endDate).getFullYear() : '';
                //   let label = member.endDateTimeStamp >= taskStartDate ?
                //     `${member.BOSP004}, last working date ${terminatedDate}` :
                //     `${member.BOSP004} terminated at ${terminatedDate}`;
                //   label = member.endDateTimeStamp == 0 ? member.BOSP004 : label;
                //   let boardNameValue = {
                //     label,
                //     value: member.id,
                //     year: `${new Date(member.startDate).getFullYear()}- ${endDate}`// ask where is year needed
                //   }
                //   let bordMemberForPayload =
                //   {
                //     label: member.BOSP004,
                //     value: member.id,
                //     year: `${new Date(member.startDate).getFullYear()}- ${endDate}`// ask where is year needed
                //   }

                //   datapointList.memberList.push(boardNameValue);
                //   memberList.push(bordMemberForPayload)

                // });

                let { memberListToSave, memberListForDisplay } = await getMembers(dpQuery, BOARD_MATRIX, taskStartDate);
                datapointList.memberList = memberListToSave;
                for (let datapointsIndex = 0; datapointsIndex < dpTypeDatapoints.length; datapointsIndex++) {
                  for (let boarMemberListIndex = 0; boarMemberListIndex < memberListForDisplay.length; boarMemberListIndex++) {
                    // initisalised object.
                    //TODO: Getting DpCodes only belonging to the request memberId.
                    if (memberListForDisplay[boarMemberListIndex].value == memberId) {
                      const memberData = await BoardMembers.findOne({ _id: memberId, status: true });
                      if (memberData.endDateTimeStamp < taskStartDate) {
                        const gender = memberData.BODR005 == 'M' ? 'he' : 'she'
                        return res.status(200).json({
                          status: 200,
                          message: `Member is terminated, ${gender} is no longer part of this task`
                        });
                      }
                      let boardDatapointsObject = getMemberDataPoint(dpTypeDatapoints[datapointsIndex], memberListForDisplay[boarMemberListIndex], taskDetails);
                      // filtered data to get status.
                      for (let currentYearIndex = 0; currentYearIndex < currentYear.length; currentYearIndex++) {
                        _.filter(currentAllBoardMemberMatrixDetails, (object) => {
                          let memberName = object.memberName;
                          let element = memberListForDisplay[boarMemberListIndex].label;
              ...
              case KMP_MATRIX:
                ...
                //   }
                // }
                let { memberListToSaveKmp, memberListForDisplayKmp } = await getMembers(dpQuery, KMP_MATRIX, taskStartDate);
                datapointList.memberList = memberListToSaveKmp;
                for (let datapointsIndex = 0; datapointsIndex < dpTypeDatapoints.length; datapointsIndex++) {
                  for (let kmpMemberListIndex = 0; kmpMemberListIndex < memberListForDisplayKmp.length; kmpMemberListIndex++) {
                    if (memberListForDisplayKmp[kmpMemberListIndex].value == memberId) {
                      const memberData = await Kmp.findOne({ _id: memberId, status: true });
                      if (memberData.endDateTimeStamp < taskStartDate) {
                        const gender = memberData.MASR008 == 'M' ? 'he' : 'she'
                        return res.status(200).json({
                          status: 200,
                          message: `Member is terminated, ${gender} is no longer part of this task`
                        });
                      }
                      let kmpDatapointsObject = getMemberDataPoint(dpTypeDatapoints[datapointsIndex], memberListForDisplayKmp[kmpMemberListIndex], taskDetails);
                      for (let currentYearIndex = 0; currentYearIndex < currentYear.length; currentYearIndex++) {
                        _.filter(currentAllKmpMatrixDetails, (object) => {
                          let memberName = object.memberName;
                          let element = memberListForDisplayKmp[kmpMemberListIndex].label;
          ...
          try {
            ...
            switch (dpType) {
              ...
              case BOARD_MATRIX:
                ...
                for (let errorDpIndex = 0; errorDpIndex < orderedDpCodes.length; errorDpIndex++) {
                  _.filter(memberListForDisplay, async (object) => {
              ...
              case KMP_MATRIX:
                ...
                //   }
                // }
                let { memberListForDisplayKmp, memberListToSaveKmp } = await getMembers(dpQuery, KMP_MATRIX, taskStartDate);
                datapointList.memberList = memberListForDisplayKmp;

                for (let errorDpIndex = 0; errorDpIndex < orderedDpCodes.length; errorDpIndex++) {
                  _.filter(memberListToSaveKmp, async (object) => {
                    let memberName = orderedDpCodes[errorDpIndex].memberName;
                    if (memberName.toLowerCase().includes((object.label).toLowerCase())) {
                      const memberData = await Kmp.findOne({ _id: memberId, status: true });
                      if (memberData.endDateTimeStamp < taskStartDate) {
          ...
          try {
            ...
            switch (dpType) {
              ...
              case BOARD_MATRIX:
                queryForHasError = memberName === '' ? queryForHasError : { ...queryForHasError, memberName: { '$regex': memberName, '$options': 'i' } };
                queryForHasError = datapointCodeQuery.length > 0 ? { ...queryForHasError, datapointId: datapointCodeQuery } : queryForHasError;
                const errorboardDatapoints = await BoardMembersMatrixDataPoints.find({
                  ...queryForHasError,
                  year: {
                    $in: currentYear
                  }
...
function getTaskStartDate(currentyear, month, date) {
  ...
  const taskStartingDate = new Date(taskStartingYear, month, 0).getDate() == date ? 1 : (Number(date) + 1);
  ...
  if (month == 12) { taskStartingYear = (Number(taskStartingYear) + 1) };
...
async function getMembers(dpQuery, dpType, taskStartDate) {
  try {
    let memberList = [], datapointMemberList = [];
    let member, terminatedDate, endDate, label, boardNameValue, bordMemberForPayload;
    switch (dpType) {
      case BOARD_MATRIX:
        member = await BoardMembers.find(dpQuery);
        member.map(member => {
          terminatedDate = new Date(member.endDateTimeStamp * 1000);
          terminatedDate = format(terminatedDate, 'MM-dd-yyyy');
          endDate = new Date(member.endDate).getFullYear() ? new Date(member.endDate).getFullYear() : '';
          label = member.endDateTimeStamp >= taskStartDate ?
            `${member.BOSP004}, last working date ${terminatedDate}` :
            `${member.BOSP004} terminated at ${terminatedDate}`;
          label = member.endDateTimeStamp == 0 ? member.BOSP004 : label;
          boardNameValue = {
          ...
          datapointMemberList.push(boardNameValue);
          memberList.push(bordMemberForPayload)

        });
        return { memberListToSave: datapointMemberList, memberListForDisplay: memberList };
      case KMP_MATRIX:
        member = await Kmp.find(dpQuery);
        member.map(member => {
          terminatedDate = new Date(member.endDateTimeStamp * 1000);
          terminatedDate = format(terminatedDate, 'MM-dd-yyyy');
          endDate = new Date(member.endDate).getFullYear() ? new Date(member.endDate).getFullYear() : '';
          label = member.endDateTimeStamp >= taskStartDate ?
            `${member.MASP003}, last working date ${terminatedDate}` :
            `${member.MASP003} terminated at ${terminatedDate}`;
          label = member.endDateTimeStamp == 0 ? member.MASP003 : label;
          boardNameValue = {
            label,
            value: member.id,
            year: `${new Date(member.startDate).getFullYear()}- ${endDate}`// ask where is year needed
        ...
        return { memberListToSaveKmp: datapointMemberList, memberListForDisplayKmp: memberList };
      default:
        return 'wrong dpType'
