          ...
          for (let dpTypeIndex = 0; dpTypeIndex < dpTypeValues.length; dpTypeIndex++) {
            try {
              ...
              if (dpTypeValues[dpTypeIndex] == 'Board Matrix') {
                let boardMemberEq = await BoardDirectors.find({ companyId: taskDetails.companyId.id, status: true });
                for (let currentYearIndex = 0; currentYearIndex < currentYear.length; currentYearIndex++) {
                  ...
                  let boardMemberGt = await BoardDirectors.find({ companyId: taskDetails.companyId.id, status: true });
                ...
                let kmpMemberEq = await BoardDirectors.find({ companyId: taskDetails.companyId.id, status: true });
                for (let currentYearIndex = 0; currentYearIndex < currentYear.length; currentYearIndex++) {
                  let yearSplit = currentYear[currentYearIndex].split('-');
                  let endDateString = yearSplit[1] + "-12-31";
                  let yearTimeStamp = Math.floor(new Date(endDateString).getTime() / 1000);
                  let kmpMemberGt = await BoardDirectors.find({ companyId: taskDetails.companyId.id, status: true });
                  console.log(1614709800, yearTimeStamp)
            ...
            for (let dIndex = 0; dIndex < displayFields.length; dIndex++) {
              if (!requiredFields.includes(displayFields[dIndex].fieldName)) {
                ...
                if (displayFields[dIndex].inputType == 'Static') {
                  ...
                  if (displayFields[dIndex].inputType == 'Select') {
                    currentValue = { value: standaloneDetail.errorCaughtByRep.additionalDetails ? standaloneDetail.errorCaughtByRep.additionalDetails[displayFields[dIndex].fieldName] : '', label: standaloneDetail.errorCaughtByRep.additionalDetails ? standaloneDetail.errorCaughtByRep.additionalDetails[displayFields[dIndex].fieldName] : '' };
                  } else {
                    currentValue = standaloneDetail.errorCaughtByRep.additionalDetails ? standaloneDetail.errorCaughtByRep.additionalDetails[displayFields[dIndex].fieldName] : '';
                  }
                }
                currentDatapointsObject.error.refData.additionalDetails.push({
                  fieldName: displayFields[dIndex].fieldName,
