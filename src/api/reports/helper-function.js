...
export async function getControveryDetails(controversery, user) {
    let numberOfControversy = await Controversy.count({ taskId: controversery.id, status: true, response: {$in: [" ", "NA", ""]}, isActive: true });
    return {
...
export function getSFDRExportOutputFields(details) {
    ...
    return {
        ...
        textSnippet: details?.textSnippet ? details?.textSnippet : '',
        sectionOfDocument: details?.additionalDetails.sectionOfDocument ? details?.additionalDetails.sectionOfDocument : '',
        keywordUsed: details?.additionalDetails.keywordUsed ? details?.additionalDetails.keywordUsed : '',
        commentG: details?.additionalDetails.commentG ? details?.additionalDetails.commentG : '',
        additionalSourceUsed: details?.additionalDetails[additionalSource]?details?.additionalDetails[additionalSource]:'',
        _id: details?._id,
