  ...
  try {
    ...
    if (req.file && req.body.taskId) {
      ...
      for (let index = 0; index < kmpMembersNameList?.length; index++) {
        ...
        let memberDetails = allKmpMembers.filter(obj => obj?.BOSP004.toLowerCase().includes(element[0].toLowerCase()))
        if (memberDetails && memberDetails?.length > 0) {
          let cmpMemberDetails = allKmpMembers.find(obj => obj?.BOSP004.toLowerCase().includes(element[0].toLowerCase()) && obj.companyId.id == taskObject.companyId.id)
          if (!cmpMemberDetails) {
            let memberObject = {
